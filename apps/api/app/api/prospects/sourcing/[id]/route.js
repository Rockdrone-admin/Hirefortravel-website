  ...
  try {
    ...
    if (matchUpdates && Object.keys(matchUpdates).length > 0) {
      ...
      const matchFields = [
        ...
        'primary_flag', 'owner', 'tags', 'followup_due_at', 'last_contacted_at', 'job_id'
      ];
      ...
      for (const field of matchFields) {
        if (matchUpdates[field] !== undefined) {
          ...
          if (hasChanged) {
            ...
            let prevVal = String(currentMatch[field] || '');
            let newVal = String(matchUpdates[field] || '');

            if (field === 'tags') {
              prevVal = JSON.stringify(currentMatch.tags || []);
              newVal = JSON.stringify(matchUpdates.tags || []);
            } else if (field === 'job_id') {
              const [oldJobRes, newJobRes] = await Promise.all([
                supabase.from('jobs').select('title').eq('id', currentMatch.job_id).maybeSingle(),
                supabase.from('jobs').select('title').eq('id', matchUpdates.job_id).maybeSingle()
              ]);
              prevVal = oldJobRes.data?.title || currentMatch.job_id || 'None';
              newVal = newJobRes.data?.title || matchUpdates.job_id || 'None';
            }

            auditLogs.push({
              prospect_id: prospectId,
              job_id: field === 'job_id' ? matchUpdates.job_id : jobId,
              changed_by: changedBy || 'Admin',
    ...
    if (auditLogs.length > 0) {
      ...
      const jobChange = auditLogs.find(log => log.metadata?.field === 'job_id');
      ...
      if (jobChange) {
        logType = 'CHANGE_JOB';
        logTitle = `updated job position for candidate ${currentProspect.name || 'Candidate'}`;
      } else if (stageChange) {
