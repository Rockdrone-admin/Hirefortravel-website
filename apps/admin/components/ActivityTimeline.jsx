...
const formatDisplayString = (str) => {
  ...
  stages.forEach(stage => {
    const regex = new RegExp(`\\b${stage}\\b`, 'g');
    if (STAGE_NAME_MAP[stage]) {
      formatted = formatted.replace(regex, STAGE_NAME_MAP[stage]);
    }
  });
  return formatted;
};
            ...
            {filteredChanges.map((change, index) => {
              ...
              const formatVal = (val, fieldName) => {
                ...
                if (fieldName === 'stage') {
                  ...
                  if (STAGE_NAME_MAP[str]) return STAGE_NAME_MAP[str];
                }
        ...
        <div className="mt-3 bg-gray-50/50 border border-gray-100 rounded-xl p-3 space-y-1.5">
          {visibleEntries.map(([key, value]) => {
            ...
            if (Array.isArray(value)) {
              ...
              if (key === 'stage' || key === 'new_stage' || key === 'prev_stage') {
                const str = renderedValue.toUpperCase();
                if (STAGE_NAME_MAP[str]) {
                  renderedValue = STAGE_NAME_MAP[str];
                }
              }
            }
                    ...
                    <div className="relative flex space-x-3">
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between items-start gap-1 sm:gap-4 flex-1 min-w-0 pt-1.5">
                        {isLoginLogout ? (
                          ...
                          <div>
                            <p className="text-sm text-gray-900">
                              <span className="font-semibold text-gray-900">{event.user_name || 'System'}</span> {formatDisplayString(event.title)}
                            </p>
